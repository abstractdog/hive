...
public class LlapTokenClient {
   ...
   * instantiated an initial token, it can be used for subsequent calls, even for fetching new tokens. There is
   * a time window, when a token is not renewed anymore in the deamon by DefaultLlapTokenManager
   * (close to max lifetime) but still valid, and a new token is available at the same time.
   */
