...
- Pull the image from DockerHub: [https://hub.docker.com/r/apache/hive/tags](https://hub.docker.com/r/apache/hive/tags).
...
##### Build Tez ApplicationMaster image

In addition to the main Hive image, `build.sh` can also build a dedicated Tez ApplicationMaster (`DAGAppMaster`) image when invoked with the `-am` flag.

For example, using explicit versions:

```shell
./build.sh -hive 4.0.0 -hadoop 3.4.1 -tez 0.10.3 -am
```
...
When `-am` is specified, an additional image is produced alongside `apache/hive:${HIVE_VERSION}`:  
`apache/hive-tez-am:${TEZ_VERSION}` (or `${repo}/hive-tez-am:${TEZ_VERSION}` if `-repo` is used).
...
#### Running the Tez ApplicationMaster container

The Tez AM image built via `build.sh -am` (using `Dockerfile-am` under the hood) can be started as a generic Tez ApplicationMaster process. All standard Tez environment is expected to be provided via configuration and environment variables; the container simply launches the `org.apache.tez.dag.app.DAGAppMaster` class.

Basic example (with a pre-built image tag `apache/hive-tez-am:${TEZ_VERSION}`):

```shell
docker run --rm \
  -e HADOOP_CONF_DIR=/etc/hadoop \
  -e TEZ_CONF_DIR=/etc/hadoop \
  apache/hive-tez-am:${TEZ_VERSION}
```

Any additional arguments passed to `docker run` after the image name will be forwarded to the `DAGAppMaster` main class. JVM options for the AM can be supplied via the `TEZ_AM_OPTS` environment variable.
