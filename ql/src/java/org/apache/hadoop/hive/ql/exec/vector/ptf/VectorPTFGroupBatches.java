...
public class VectorPTFGroupBatches extends PTFPartition {
  ...
  /**
   * Initializes VectorPTFGroupBatches.
   *
   * Regarding key/non-key/order columns, below scenarios are possible:
   ...
   * keyInputColumnMap: []
   * orderColumnMap: [7]
   * nonKeyInputColumnMap: [1, 2, 3, 4]
   * @param keyInputColumnMap
   * @param orderColumnMap
   * @param scratchColumnPositions
   * @return
   */
  public void init(TypeInfo[] reducerBatchTypeInfos, VectorPTFEvaluatorBase[] evaluators,
      int[] outputProjectionColumnMap, TypeInfo[] outputTypeInfos, int[] orderColumnMap,
      Type[] orderColumnVectorTypes, int[] keyInputColumnMap, int[] nonKeyInputColumnMap,
      int[] streamingEvaluatorNums, int[] scratchColumnPositions, String[] scratchColumnTypeNames,
      VectorizedRowBatch overflowBatch) {
    this.evaluators = evaluators;
    this.outputProjectionColumnMap = outputProjectionColumnMap;
    this.keyWithoutOrderColumnMap = determineKeyColumnsWithoutOrderColumns(keyInputColumnMap, orderColumnMap);
    /*
     ...
     */
    this.orderColumnCount = orderColumnMap.length;
    ...
    bufferedColumnCount = orderColumnCount + nonKeyInputColumnCount + streamingEvaluatorCount + scratchColumnPositions.length;
    ...
    for (int i = 0; i < scratchColumnPositions.length; i++) {
      ...
      bufferedTypeInfos[bufferedMapIndex] = TypeInfoUtils.getTypeInfoFromTypeString(scratchColumnTypeNames[i]);
    }
    ...
    for (int i = 0; i < evaluators.length; i++) {
      ...
      // NON-STREAMING evaluators work on buffered batches, need to adapt them
   ...
   * Overrides PTFPartition.getAt()
   */
  @Override
  public Object[] getAt(int i) throws HiveException {
    RowPositionInBatch rp = getPosition(i);
    return getValue(bufferedBatches.get(rp.batchIndex), rp.rowIndexInBatch);
  }
  ...
  public void bufferGroupBatch(VectorizedRowBatch batch, boolean isLastGroupBatch) throws HiveException {
    ...
    copyBufferedColumns(batch, bufferedBatch, bufferedColumnCount);
  ...
  private TypeInfo columnVectorTypeToTypeInfo(Type type) {
    switch (type) {
    case DOUBLE:
  ...
  private Object[] getValue(VectorizedRowBatch vectorizedRowBatch, int rowIndexInBatch) {
    Object[] orderingValues = new Object[orderColumnCount];
  ...
  private void runEvaluatorForRow(int evaluatorIndex, VectorPTFEvaluatorBase evaluator,
    ...
    if (evaluator.canRunOptimizedCalculation(rowNum, range)) {
      result = evaluator.runOnRange(rowNum, range, this);
      partitionMetrics.evaluationOptimized += 1;
    } else { // fallback to batch-by-batch manner over the range
      result = runEvaluatorOnRange(evaluator, range);
  ...
  public Object runEvaluatorOnRange(VectorPTFEvaluatorBase evaluator, Range range)
    ...
    if (evaluator.getResultColumnVectorType() == Type.DECIMAL) {
      /*
       * Unfortunately, as HiveDecimalWritable is mutable, we need to copy the value before
