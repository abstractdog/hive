...
class VectorSpillBlockContainer {
  ...
  List<VectorSpillBlock> blocks = new ArrayList<>();
  ...
  /**
   * Should contain the index of the first batch which is now in memory
   * [S0, S1] [M2, M3] -> 2
   */
  private int startBatchIndexInMemory = 0;

  /**
   * VectorSpillBlock is an abstraction of spilled vector batches.
   * A block contains spillLimitBufferedBatchCount batches.
   ...
   *
   * 3. moving backward (previousBlock), want to have S2,S3 in the memory, so need to spill M4 if it's not spilled earlier
   * [S0 S1] [S2 S3] [S4   ] [M2 M3]
   *
   * 4. moving forward (nextBlock), want to have S4 in the memory again, simplry
   * [S0 S1] [S2 S3] [S4   ] [M4 ]
   */
  ...
  class VectorSpillBlock {
     ...
     * rows, this needs to be tracked, as this CANNOT be calculated simply by spillBatchCount * "some
     * batch size"
     */
    @VisibleForTesting
    long spillRowCount;
     ...
     * the block is full, this is supposed to be equal to spillLimitBufferedBatchCount
     */
    int spillBatchCount;
     ...
     * ...
     * n.th block:  startBatchIndex = n * spillLimitBufferedBatchCount
     */
  ...
  public boolean isStandingOnFirst() {
